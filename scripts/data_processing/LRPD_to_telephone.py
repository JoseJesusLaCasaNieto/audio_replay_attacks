"""
LRPD_to_telephone.py

This script processes the LRPD audio dataset and generates a telephone-band version
...
source_root = Path('/mnt/media/fair/audio/replay_attacks/datasets/LRPD')
destination_root = Path('/mnt/media/fair/audio/replay_attacks/datasets/LRPD_telephone')
...
def main():
    ...
    for subdir in SUBDIRS:
        ...
        for codec, files in assignment.items():
            label = CODECS[codec]
            other_codec = "pcm_alaw" if codec == "pcm_mulaw" else "pcm_mulaw"
            other_label = CODECS[other_codec]
            ...
            for input_file in tqdm(
                ...
                other_file = out_dir / f"{stem}_telephone_{other_label}.wav"

                if out_file.exists() or other_file.exists():
                    continue
