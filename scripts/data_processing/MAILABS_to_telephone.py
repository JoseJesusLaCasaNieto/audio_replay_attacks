"""
MAILABS_to_telephone.py

This script processes the ReplayDF audio dataset and generates a telephone-band version
...
Usage:
    python MAILABS_to_telephone.py
...
source_root = Path('/mnt/media/fair/audio/replay_attacks/datasets/MAILABS')
destination_root = Path('/mnt/media/fair/audio/replay_attacks/datasets/MAILABS_telephone')
...
def main():
    ...
    logger.info(f"Processing directory: {source_root}")
    if not source_root.exists():
        logger.warning(f"Directory not found, skipping: {source_root}")

    # Gather all audio files
    audio_files = [p for p in source_root.rglob('*') if p.suffix.lower() in EXTENSIONS]
    logger.info(f"Found {len(audio_files)} files in {source_root}")
    if not audio_files:
        logger.warning(f"No audio files found in: {source_root}")
    ...
    for codec, files in assignment.items():
        label = CODECS[codec]
        for input_file in tqdm(files, unit='audio', desc='Converting wavs to telephone'):
    ...
    logger.success(f"Directory processed succesfully: {source_root}")

    logger.success('==== Telephone dataset generation completed successfully ====')
